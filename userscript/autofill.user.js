// ==UserScript==
// @name         Auto Fill — Payment Forms
// @namespace    https://github.com/your/autofill
// @version      1.0.0
// @description  Auto-fill payment forms (Stripe Elements, Checkout, plain). Injects into Stripe iframes via @match.
// @match        https://*/*
// @match        http://localhost/*
// @match        http://127.0.0.1/*
// @match        https://js.stripe.com/v3/*
// @match        https://js.stripe.com/v3/elements-inner-*
...
(function (root) {
  ...
  // ─── Profile store (edit via Tampermonkey menu: "Edit profiles") ───
  ...
  function loadProfiles() {
    try {
      const raw = GM_getValue('profiles', null);
      if (!raw) { GM_setValue('profiles', JSON.stringify(DEFAULT_PROFILES)); return DEFAULT_PROFILES; }
      return JSON.parse(raw);
    } catch (e) {
      console.warn('[autofill] profile load failed, using defaults', e);
  ...
  function run(profileKey, submit) {
    const store = loadProfiles();
    const key = profileKey || store.defaultProfile;
    const profile = store.profiles[key];
    if (!profile) { toast('[autofill] profile not found: ' + key); return; }

    const isStripeFrame = location.hostname === 'js.stripe.com';
    // Never auto-submit from inside an iframe — the submit button lives in the top frame.
    const result = window.__AutoFill.fill(profile, { submit: submit && !isStripeFrame && window.top === window });
    console.log('[autofill]', location.hostname, result);

    if (window.top === window) {
      toast(`Filled ${result.filled.length} · missed ${result.missed.join(', ') || 'none'}`);
      // Fan out to nested frames so the userscript in each Stripe iframe fills its own fields.
      try {
        document.querySelectorAll('iframe').forEach(function (f) {
          try { f.contentWindow.postMessage({ __autofill: 'fill', profileKey: key }, '*'); } catch (_) {}
        });
      } catch (_) {}
    }
  }

  // Register menu commands for each profile.
  const store = loadProfiles();
  for (const key of Object.keys(store.profiles)) {
    const label = store.profiles[key].label || key;
    GM_registerMenuCommand(`Fill — ${label}`,        function () { run(key, false); });
    GM_registerMenuCommand(`Fill + Submit — ${label}`, function () { run(key, true); });
  }
  GM_registerMenuCommand('Edit profiles (JSON)', function () {
    const current = GM_getValue('profiles', JSON.stringify(DEFAULT_PROFILES, null, 2));
    const next = prompt('Paste full profiles JSON:', current);
    if (next) {
      try { JSON.parse(next); GM_setValue('profiles', next); toast('Profiles saved — reload page'); }
      catch (e) { alert('Invalid JSON: ' + e.message); }
    }
  });

  if (location.hostname === 'js.stripe.com') {
    // Tampermonkey runs this script independently in each Stripe iframe.
    // The top-frame instance broadcasts a postMessage; we listen and fill our own frame.
    window.addEventListener('message', function (ev) {
      if (ev.data && ev.data.__autofill === 'fill') {
        run(ev.data.profileKey, false);
      }
    });
  }
})();
